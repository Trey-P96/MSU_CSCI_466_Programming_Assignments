...
class MPLSFrame:
    mpls = 'MPLS'
    label_length = len(mpls)

    def __init__(self, pkt_s):
        self.pkt_s = pkt_s

    def to_byte_S(self):
        byte_S = str(self.mpls).zfill(self.label_length)
        byte_S += self.pkt_s
        return byte_S

    @classmethod
    def from_byte_S(self, mpls_frame_S):
        pkt_s = mpls_frame_S[MPLSFrame.label_length:]
        return pkt_s
...
class Router:
    ...
    def process_queues(self):
        for i in range(len(self.intf_L)):
            ...
            if fr_S is None:
                continue # no frames to process yet
            fr = LinkFrame.from_byte_S(fr_S)
            pkt_S = fr.data_S
    ...
    def process_network_packet(self, pkt, i):
        ...
        # for now, we just relabel the packet as an MPLS frame without encapsulation
        mpls_Fr = None
        for key, intf in self.encap_tbl_D.items():
            if key is self.name and intf is i:
                pkt_s = str(pkt)
                mpls_Fr = MPLSFrame(pkt_s)
                mpls_frame = mpls_Fr.to_byte_S()
                print('\nNODE %s IN INTERFACE: %d\n\nNODE %s: MPLS\n\nPACKET %s: MPLS: %s\n'
                      % (self.name, intf, self.name, pkt_s, mpls_frame))
            elif key is self.name and intf is not i:
                mpls_Fr = pkt
        m_fr = mpls_Fr
    ...
    def process_MPLS_frame(self, m_fr, i):
        ...
        try:
            for node, val in self.frwd_tbl_D.items():
                if node is self.name:
                    self.process(i, val, m_fr)
        except queue.Full:
